...
void DungeonLead::Stop(PlayerbotAI* botAI, bool giveLeaderBack)
{
    ...
    botAI->ChangeStrategy("-dungeon lead", BOT_STATE_COMBAT);
...
bool DungeonLeadNextAction::MoveRouteTo(DungeonLeadState& st, WorldPosition const& dest, DungeonRouteStep const& step)
{
    ...
    // blocked: sample the forward cone for a reachable stepping stone
    float minDelta = M_PI;
    ...
    {
        float delta = (rand_norm() - 0.5f) * static_cast<float>(M_PI);
        float sampleDis = (0.5f + rand_norm() * 0.5f) * kPathFinderDis;
